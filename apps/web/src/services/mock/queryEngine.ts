/**
 * mock 的"服务端语义"：过滤 / 聚合 / 排序 / 分页。
 * 未来接真实后端时，这些函数可以直接搬到 server 侧复用，保证前后端口径一致。
 */
...
function sumFacts(facts: MockDailyFact[]): {
  impressions: number;
  clicks: number;
  conversions: number;
  orders: number;
...
export function filterFacts(facts: MockDailyFact[], query: DashboardQuery): MockDailyFact[] {
  const channelSet =
...
export function aggregateMetrics(facts: MockDailyFact[]): MetricSummary {
  if (facts.length === 0) {
...
export function buildTrend(facts: MockDailyFact[], from: string, to: string): TrendPoint[] {
...
export function buildChannelMetrics(facts: MockDailyFact[]): ChannelMetric[] {
  const grouped = new Map<AdChannel, MockDailyFact[]>();
...
export function buildFunnel(facts: MockDailyFact[]): FunnelStage[] {
  const totals = sumFacts(facts);
...
/** 按 计划 + 渠道 聚合出明细表行 */
export function aggregateRecords(facts: MockDailyFact[]): AdPlanRecord[] {
  const grouped = new Map<string, MockDailyFact[]>();
